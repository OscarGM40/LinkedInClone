...
const Post = ({ post, modalPost }: PostProps) => {
  ...
  const [, setModalOpen] = useRecoilState<boolean>(modalState);
  const [, setModalType] = useRecoilState<string>(modalTypeState); // gifYouUp | dropIn
  const [, setPostState] = useRecoilState(getPostState);
  const [showInput, setShowInput] = useState(false); //show more funcionality
    ...
    <div className={`bg-white dark:bg-[#1D2226] ${modalPost ? "rounded-r-lg" : "rounded-lg"} space-y-2 py-2.5 border border-gray-300 dark:border-none`}>
      ...
      {post.input && (
        <div className="px-2.5 break-all md:break-normal">
          {modalPost || showInput ? (
            <p className="" onClick={() => setShowInput(false)}>{post.input}</p>
