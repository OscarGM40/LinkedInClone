...
interface Props {
  handleClose: React.EventHandler<any> | any,
  type: string | any,
}
const Modal = ({ handleClose, type }: Props) => {
  const { data: session } = useSession();
  const post = useRecoilValue<PostResponse>(getPostState)
    ...
    <Backdrop onClick={handleClose}>
        ...
        >
          ...
          <div className="p-4 space-y-2">
            <div className="flex items-center space-x-2">
              <Avatar src={session?.user?.image || "/images/default-image.jpg"} className="!h-11 !w-11" />
      ...
      {type === "gifYouUp" && (
        ...
        <motion.img
            ...
            src={post.photoUrl || "/images/default-image.jpg"}
            className="object-contain max-h-[80vh] w-full max-w-3xl rounded-l-lg"
          />
          <div className="w-full md:w-3/5 bg-white dark:bg-[#1D2226] rounded rounded-r-lg">
